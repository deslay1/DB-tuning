...
class RocksdbBenchmark:
    def __init__(self, bench_type=None, options={}):
        ...
        if 'readwritepercent' in options: self.__readwritepercent = int(options['readwritepercent'])
    ...
    def run_benchmark(self, runs=1, num_million=1, fill=False, options_file=False, max_seconds=300, use_existing=False):
        ...
        if self.__readwritepercent is not None: command += f' -readwritepercent={self.__readwritepercent}'
        print(command)
